...
{
    static_assert(std::is_base_of<BindingEvaluator, EvaluatorT>::value,
...
public:
    ...
    explicit Binding(Private::Node<T> &&rootNode, EvaluatorT const &evaluator) :
        m_rootNode{std::move(rootNode)}, m_evaluator{evaluator}
    {
 ...
 * ["Reassigning a Binding"](../../getting-started/data-binding/#reassigning-a-binding) section in
 * the Getting Started guide.*
 */
template <typename T, typename EvaluatorT>
inline std::unique_ptr<Binding<T, EvaluatorT>> makeBinding(EvaluatorT &evaluator,
                                                           Property<T> &property)
{
 ...
 * @return std::unique_ptr<Binding<T, EvaluatorT>> A new Binding that combines the rootNode with the
 * evaluator.
 *
 * *Note: For the difference between makeBinding and makeBoundProperty, see the
 * ["Reassigning a Binding"](../../getting-started/data-binding/#reassigning-a-binding) section in
 ...
 *              They will be automatically unwrapped, i.e. a Property<T> will pass a value of type T
 * to func.
 * @return std::unique_ptr<Binding<ReturnType, EvaluatorT>> where ReturnType is the type that
 * results from evaluationg func with the given arguments. The Binding will be powered by the new
 * evaluator.
 *
 * *Note: For the difference between makeBinding and makeBoundProperty, see the
 * ["Reassigning a Binding"](../../getting-started/data-binding/#reassigning-a-binding) section in
 * the Getting Started guide.*
 */
template <typename EvaluatorT, typename Func, typename... Args,
          typename = std::enable_if_t<sizeof...(Args) != 0>,
          typename ResultType = Private::operator_node_result_t<Func, Args...>>
inline std::unique_ptr<Binding<ResultType, EvaluatorT>> makeBinding(EvaluatorT &evaluator,
                                                                    Func &&func, Args &&...args)
{
    return std::make_unique<Binding<ResultType, EvaluatorT>>(
 ...
 * @return std::unique_ptr<Binding<<T, ImmediateBindingEvaluator>> An new Binding bound to a root
 * Node with immediate evaluation.
 *
 * *Note: For the difference between makeBinding and makeBoundProperty, see the
 * ["Reassigning a Binding"](../../getting-started/data-binding/#reassigning-a-binding) section in
 * the Getting Started guide.*
 */
template <typename T>
inline std::unique_ptr<Binding<T, ImmediateBindingEvaluator>>
 ...
 *              They will be automatically unwrapped, i.e. a Property<T> will pass a value of type T
 * to func.
 * @return std::unique_ptr<Binding<ReturnType, ImmediateBindingEvaluator>> where ReturnType is the
 * type that results from evaluationg func with the given arguments. The Binding will feature
 ...
 * the Getting Started guide.*
 */
template <typename Func, typename... Args, typename = std::enable_if_t<sizeof...(Args) != 0>,
          typename ResultType = Private::operator_node_result_t<Func, Args...>>
inline std::unique_ptr<Binding<ResultType, ImmediateBindingEvaluator>> makeBinding(Func &&func,
                                                                                   Args &&...args)
{
    return std::make_unique<Binding<ResultType, ImmediateBindingEvaluator>>(
