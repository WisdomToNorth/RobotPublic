/*
  ...
  <info@kdab.com> Author: Sean Harmer <sean.harmer@kdab.com>
...
public:
     ...
     * In this case the ConnectionHandle will not reference any active connection (i.e. isActive()
     *will return false), and not belong to any Signal.
     **/
     ...
     * @throw std::out_of_range Throws if the connection is not active (i.e. isActive() returns
     *false).
     **/
    ...
    bool belongsTo(const Signal<Args...> &signal) const
    {
        ...
        return shared_impl
               && shared_impl == std::static_pointer_cast<Private::SignalImplBase>(signal.m_impl);
    }
...
private:
    ...
    ConnectionHandle(std::weak_ptr<Private::SignalImplBase> signalImpl,
                     Private::GenerationalIndex id) :
 ...
 *
 * KDBindings::Signal recreates the <a href="https://doc.qt.io/qt-5/signalsandslots.html">Qt's
 * Signals & Slots mechanism</a> in pure C++17. A Signal can be used to notify any number of slots
...
class Signal
{
    static_assert(std::conjunction<std::negation<std::is_rvalue_reference<Args>>...>::value,
    ...
    public:
        ...
        {
            return m_connections.get(id) != nullptr;
        }
        ...
        void emit(Args... p) const
        {
            ...
            // but adding new connections to a signal inside a slot will still be undefined
            // behaviour
...
public:
    ...
    ConnectionHandle connect(std::function<void(Args...)> const &slot)
    {
        ...
        auto impl = m_impl;
        auto connection = impl->connect(slot);
        return ConnectionHandle(impl, connection);
    }

    /**
     * A template overload of Signal::connect that makes it easier to connect arbitrary functions to
     *this Signal. It connects a function to this Signal, binds any provided arguments to that
     *function and discards any values emitted by this Signal that aren't needed by the resulting
     *function.
     *
     ...
     *          Warning: When connecting a member function you must use the returned
     *ConnectionHandle to disconnect when the object containing the slot goes out of scope!
     **/
    ...
    template <
        typename Func, typename... FuncArgs,
        typename =
            std::enable_if_t<std::disjunction_v<std::negation<std::is_convertible<
                                                    Func, std::function<void(Args...)>>>,
                                                std::integral_constant<bool, sizeof...(FuncArgs) /*Also enable this function if we want to bind at least one argument*/>>>>
    ConnectionHandle connect(Func &&slot, FuncArgs &&...args)
    {
        std::function<void(Args...)> bound =
...
/**
 * @brief A ConnectionBlocker is a convenient RAII-style mechanism for temporarily blocking a
 * connection.
 *
...
public:
     ...
     * refers to.
     *
     * @throw std::out_of_range If the connection is not active (i.e. ConnectionHandle::isActive()
     * returns false).
     */
 ...
 *
 * An example of how to use the KDBindings::Signal::connect() overloaded function for advanced slot
 * connections.
 *
 * The output of this example is:
 * ```
