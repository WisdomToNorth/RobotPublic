/*
  ...
  <info@kdab.com> Author: Leon Matthes <leon.matthes@kdab.com>
...
// Add + 1 to sizeof...(Arguments) here as the "this" pointer is an implicit argument to any member
// function.
#define KDBINDINGS_DEFINE_MEMBER_GET_ARITY(MODIFIERS)                                              \
    template <typename Return, typename Class, typename... Arguments>                              \
    constexpr size_t get_arity(                                                                    \
        ::KDBindings::Private::TypeMarker<Return (Class::*)(Arguments...) MODIFIERS>)              \
    {                                                                                              \
...
{
    return std::bind(std::forward<Func>(fun), std::forward<Args>(args)...,
                     placeholder<Is + 1>{}...);
}

// bind_first binds the first arguments to the callable object (i.e. function) to the values
// provided by args. The return value is a new function taking get_arity<Func> - sizeof...(Args)
// many arguments, with the first sizeof...(Args) arguments bound to the values of args. This is
// different to a call with std::bind(fun, args...), as the callable object created by std::bind
// would in this case now take zero arguments, whilst bind_first still expects the remaining
// arguments to be provided
//
// For now, providing instances of std::placeholders in Args is not allowed, as the implications of
// this are unclear if sizeof...(Args) != get_arity<Func>. The enable_if_t makes sure none of the
// Args value is a placeholder.
//
// In the future, we could provide another overload of this function that allows placeholders, as
// long as all arguments are bound.
template <
    typename Func, typename... Args,
    /*Disallow any placeholder arguments, they would mess with the number and ordering of required
       and bound arguments, and are, for now, unsupported*/
    typename = std::enable_if_t<std::conjunction_v<std::negation<std::is_placeholder<Args>>...>>>
auto bind_first(Func &&fun, Args &&...args)
{
    return bind_first_helper(std::make_index_sequence<get_arity<Func>() - sizeof...(Args)>{},
...
template <int N>
struct is_placeholder<KDBindings::Private::placeholder<N>> : integral_constant<int, N>
{
};
