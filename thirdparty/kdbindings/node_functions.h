...
struct any_bindables<HEAD, Ts...>
    : std::integral_constant<bool, any_bindables<HEAD>::value || any_bindables<Ts...>::value>
{
};
...
/**
 * @brief KDBINDINGS_DECLARE_FUNCTION is a helper macro to declare and define functions for use in
 * data binding.
 *
 * This macro can take any callable object or function reference and create a new function that may
 * be used in data binding expressions. The result function that can be called with a Property or
 * the result of a data binding expression to create another data binding expression.
 *
 * Note that if a function is overloaded, it is impossible to reference all of its overloads at
 * once. Therefore we recommend declaring a struct with a templated operator() to use as the
 * function object. See the KDBindings::node_abs struct for an example of how to do this.
 *
...
#define KDBINDINGS_DECLARE_FUNCTION(NAME, FUNC)                                                    \
    template <typename... Ts>                                                                      \
    inline auto NAME(Ts &&...args)                                                                 \
        -> std::enable_if_t<KDBindings::Private::any_bindables<Ts...>::value,                      \
                            KDBindings::Private::Node<KDBindings::Private::operator_node_result_t< \
                                decltype(FUNC), Ts...>>>                                           \
    {                                                                                              \
...
/**
 * @brief An example struct that is used with a call to KDBINDINGS_DECLARE_FUNCTION to declare all
 * overloads of std::abs as usable in data binding.
 *
 * Because of the way node_abs overloads its operator(), it can be used in a call to
 * KDBINDINGS_DECLARE_FUNCTION like this:
 * @code
