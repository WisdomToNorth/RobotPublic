...
#define KDBINDINGS_DEFINE_BINARY_OP(OP)                                                            \
    ...
    {                                                                                              \
        return Private::makeNode([](auto &&av, auto &&bv) { return (av OP bv); }, a,               \
                                 std::forward<B>(b));                                              \
    }                                                                                              \
                                                                                                   \
    ...
    {                                                                                              \
        return Private::makeNode([](auto &&av, auto &&bv) { return (av OP bv); },                  \
                                 std::forward<A>(a), b);                                           \
    }                                                                                              \
                                                                                                   \
    template <typename A, typename B>                                                              \
    ...
    {                                                                                              \
        return Private::makeNode([](auto &&av, auto &&bv) { return (av OP bv); }, std::move(a),    \
                                 std::forward<B>(b));                                              \
    }                                                                                              \
                                                                                                   \
    template <typename A, typename B>                                                              \
    ...
    {                                                                                              \
        return Private::makeNode([](auto &&av, auto &&bv) { return (av OP bv); },                  \
                                 std::forward<A>(a), std::move(b));                                \
                                                                                                   ...
                                                                                                   \
    template <typename A, typename B>                                                              \
    inline auto operator OP(Private::Node<A> &&a,                                                  \
                            Private::Node<B> &&b) noexcept(noexcept(a.evaluate() OP b.evaluate())) \
    ...
    {                                                                                              \
        return Private::makeNode([](auto &&av, auto &&bv) { return (av OP bv); }, std::move(a),    \
                                 std::move(b));                                                    \
                                                                                                   ...
                                                                                                   \
    template <typename B, typename A>                                                              \
    inline auto operator OP(Property<A> &a, Private::Node<B> &&b) noexcept(                        \
        noexcept(a.get() OP b.evaluate())) -> Private::Node<decltype(a.get() OP b.evaluate())>     \
    {                                                                                              \
        return Private::makeNode([](auto &&av, auto &&bv) { return (av OP bv); }, a,               \
                                 std::move(b));                                                    \
    }                                                                                              \
                                                                                                   \
    template <typename A, typename B>                                                              \
    inline auto operator OP(Private::Node<A> &&a, Property<B> &b) noexcept(                        \
        noexcept(a.evaluate() OP b.get())) -> Private::Node<decltype(a.evaluate() OP b.get())>     \
    {                                                                                              \
        return Private::makeNode([](auto &&av, auto &&bv) { return (av OP bv); }, std::move(a),    \
                                 b);                                                               \
    }
