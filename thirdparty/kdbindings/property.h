...
struct are_equality_comparable<
    ...
    std::enable_if_t<std::is_same<
        std::decay_t<decltype(std::equal_to<>{}(std::declval<X>(), std::declval<Y>()))>,
 ...
 *
 * To create a property from a data binding expression, use the @ref makeBoundProperty or @ref
 * makeBinding functions in the @ref KDBindings namespace.
 *
 * Examples:
...
public:
    ...
    Property(Property<T> &&other) noexcept(std::is_nothrow_move_constructible<T>::value) :
        ...
        m_valueChanged(std::move(other.m_valueChanged)), m_destroyed(std::move(other.m_destroyed)),
...
private:
    ...
    mutable Signal<const T &> m_valueChanged; // By const ref so we can emit the signal for
                                              // move-only types of T e.g. std::unique_ptr<int>
 ...
 *
 * An example of how to create a KDBindings::Property and use its valueChanged() KDBindings::Signal
 * to receive notifications whenever the value of the KDBindigns::Property changes.
 *
 ...
 *
 * An example of how to use makeBoundProperty() to create a KDBindings::Property that is
 * automatically updated once any of its inputs change.
 *
 * The output of this example is:
 * ```
