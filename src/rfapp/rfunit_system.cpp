...
static UnitSystem::TranslateResult translateSI(double value, Unit unit)
{
    switch (unit)
    {
    case Unit::Length: return {value, "mm", 1.};
    case Unit::Area: return {value, "mm²", 1.};
    case Unit::Volume: return {value, "mm³", 1.};
    case Unit::Velocity: return {value, "mm/s", 1.};
    case Unit::Density: return {value, "kg/m³", 1.};
    case Unit::Pressure: return {value, "kPa", 1.};
    default: return {value, symbol(unit), 1.};
    }
}
...
static UnitSystem::TranslateResult translateImperialUK(double value, Unit unit)
{
    switch (unit)
    {
    case Unit::Length: return {value / 25.4, "in", 25.4};
    case Unit::Area: return {value / 645.16, "in²", 654.16};
    case Unit::Volume: return {value / 16387.064, "in³", 16387.064};
    case Unit::Velocity: return {value / (25.4 / 60.), "in/min", 25.4 / 60.};
    default: return {value, symbol(unit), 1.};
    }
}

template <typename QuantityType>
UnitSystem::TranslateResult translateHelper(QuantityType qty, QuantityType qtyFactor,
                                            const char *strUnit)
{
...
UnitSystem::TranslateResult UnitSystem::translate(Schema schema, double value, Unit unit)
{
    switch (schema)
    {
    case Schema::SI: return Internal::translateSI(value, unit);
    case Schema::ImperialUK: return Internal::translateImperialUK(value, unit);
    }
...
UnitSystem::TranslateResult UnitSystem::parseQuantity(std::string_view strQuantity, Unit *ptrUnit)
{
    ...
    double v;
    auto res =
        fast_float::from_chars(strQuantity.data(), strQuantity.data() + strQuantity.size(), v);
    if (res.ec != std::errc())
        return {};
...
UnitSystem::TranslateResult UnitSystem::translateLength(QuantityLength length, LengthUnit unit)
{
    ...
    case LengthUnit::NauticalMile:
        return fnTrResult(Quantity_NauticalMile, "nmi");
        // Imperial UK
...
UnitSystem::TranslateResult UnitSystem::translateArea(QuantityArea area, AreaUnit unit)
{
    ...
    case AreaUnit::SquareKilometer:
        return fnTrResult(Quantity_SquareKilometer, "km²");
        // Imperial UK
