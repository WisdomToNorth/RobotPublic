...
#pragma once
#ifndef RF_QUANTITY_H
#define RF_QUANTITY_H
...
public:
    ...
    constexpr void setValue(double v)
    {
        m_value = v;
    }
    ...
    constexpr Qty operator/(double v) const
    {
        return Qty(m_value / v);
    }
    constexpr Qty operator+(Qty q) const
    {
        return Qty(m_value + q.value());
    }
    constexpr Qty operator-(Qty q) const
    {
        return Qty(m_value - q.value());
    }
    constexpr Qty operator-() const
    {
        return Qty(-m_value);
    }

    constexpr bool operator==(Qty q) const
    {
        return m_value == q.value();
    }
    constexpr bool operator<(Qty q) const
    {
        return m_value < q.value();
    ...
    constexpr Qty &operator+=(Qty q)
    {
        return this->change(m_value + q.value());
    }
    constexpr Qty &operator-=(Qty q)
    {
        return this->change(m_value - q.value());
...
template <Unit U>
constexpr Quantity<U> operator*(Quantity<U> lhs, double rhs);
template <Unit U>
constexpr Quantity<U> operator*(double lhs, Quantity<U> rhs);
template <Unit U>
constexpr double operator/(Quantity<U> lhs, Quantity<U> rhs);
