...
static const QCursor &rotateCursor()
{
    ...
    if (cursor.shape() == Qt::ArrowCursor)
    {
        ...
        static unsigned char cursorBitmap[] = {0xf0, 0xef, 0x18, 0xb8, 0x0c, 0x90, 0xe4, 0x83,
                                               0x34, 0x86, 0x1c, 0x83, 0x00, 0x81, 0x00, 0xff,
                                               0xff, 0x00, 0x81, 0x00, 0xc1, 0x38, 0x61, 0x2c,
                                               0xc1, 0x27, 0x09, 0x30, 0x1d, 0x18, 0xf7, 0x0f};
        static unsigned char cursorMaskBitmap[] = {0xf0, 0xef, 0xf8, 0xff, 0xfc, 0xff, 0xfc, 0xff,
...
protected:
    void paintEvent(QPaintEvent *) override
    {
        ...
        const QColor lineColor = QColor(65, 200, 250);
        QPalette palette = this->palette();
        QColor fillColor = palette.color(QPalette::Highlight).lighter();
    ...
    , m_actionMatcher(createActionMatcher(m_navigStyle, &m_inputSequence))
{
    ...
    m_inputSequence.setPrePushCallback([this](Input in) { m_actionMatcher->onInputPrePush(in); });
    m_inputSequence.setPreReleaseCallback([this](Input in)
                                          { m_actionMatcher->onInputPreRelease(in); });
    m_inputSequence.setClearCallback([this] { m_actionMatcher->onInputCleared(); });
}
...
void WidgetOccViewController::handleEvent(const QEvent *event)
{
    switch (event->type())
    {
    case QEvent::KeyPress: this->handleKeyPress(static_cast<const QKeyEvent *>(event)); break;
    case QEvent::KeyRelease: this->handleKeyRelease(static_cast<const QKeyEvent *>(event)); break;
    case QEvent::MouseButtonPress:
        this->handleMouseButtonPress(static_cast<const QMouseEvent *>(event));
        break;
    case QEvent::MouseMove: this->handleMouseMove(static_cast<const QMouseEvent *>(event)); break;
    case QEvent::MouseButtonRelease:
        this->handleMouseButtonRelease(static_cast<const QMouseEvent *>(event));
        break;
    case QEvent::Wheel: this->handleMouseWheel(static_cast<const QWheelEvent *>(event)); break;
    default: break;
    } // end switch
}
...
void WidgetOccViewController::handleKeyRelease(const QKeyEvent *event)
{
    ...
    if (m_inputSequence.lastInput() == Qt::Key_Space &&
...
namespace
{
static QPoint getMouseEventGlobalPoint(const QMouseEvent *event)
{
...
void WidgetOccViewController::handleMouseButtonPress(const QMouseEvent *event)
{
    ...
    const QPoint currPos = m_occView->widget()->mapFromGlobal(getMouseEventGlobalPoint(event));
...
void WidgetOccViewController::handleMouseMove(const QMouseEvent *event)
{
    const Position currPos =
        toPosition(m_occView->widget()->mapFromGlobal(getMouseEventGlobalPoint(event)));
...
{
public:
    Mayo_ActionMatcher(const InputSequence *seq)
        : ActionMatcher(seq)
    {
    }
...
{
public:
    Catia_ActionMatcher(const InputSequence *seq)
        : ActionMatcher(seq)
    {
        m_timer.start();
    }
...
{
public:
    SolidWorks_ActionMatcher(const InputSequence *seq)
        : ActionMatcher(seq)
    {
    }
...
{
public:
    Unigraphics_ActionMatcher(const InputSequence *seq)
        : ActionMatcher(seq)
    {
    }
...
{
public:
    ProEngineer_ActionMatcher(const InputSequence *seq)
        : ActionMatcher(seq)
    {
    }
...
WidgetOccViewController::createActionMatcher(View3dNavigationStyle style, const InputSequence *seq)
{
    ...
    case View3dNavigationStyle::Unigraphics:
        return std::make_unique<Unigraphics_ActionMatcher>(seq);
    case View3dNavigationStyle::ProEngineer:
        return std::make_unique<ProEngineer_ActionMatcher>(seq);
    }
    return {};
}
