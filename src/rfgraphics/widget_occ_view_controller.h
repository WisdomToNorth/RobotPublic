...
private:
    static Position toPosition(const QPoint &pnt)
    {
        return {pnt.x(), pnt.y()};
    }
    static QPoint toQPoint(const Position &pos)
    {
        return {pos.x, pos.y};
    }
    ...
    public:
        ...
        void clear();
        std::span<const Input> data() const
        {
            return m_inputs;
        }
        ...
        void setPrePushCallback(std::function<void(Input)> fn)
        {
            m_fnPrePushCallback = std::move(fn);
        }
        void setPreReleaseCallback(std::function<void(Input)> fn)
        {
            m_fnPreReleaseCallback = std::move(fn);
        }
        void setClearCallback(std::function<void()> fn)
        {
            m_fnClearCallback = std::move(fn);
        }
    ...
    {
    public:
        ActionMatcher(const InputSequence *seq)
            : inputs(*seq)
        {
        }
    ...
    static std::unique_ptr<ActionMatcher> createActionMatcher(View3dNavigationStyle style,
