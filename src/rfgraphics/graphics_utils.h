...
struct GraphicsUtils
{
    ...
    static bool V3dView_hasClipPlane(const OccHandle<V3d_View> &view,
    ...
    static void V3dViewer_setGridColors(const OccHandle<V3d_Viewer> &viewer,
                                        const AspectGridColors &colors);

    static void AisContext_eraseObject(const OccHandle<AIS_InteractiveContext> &context,
                                       const OccHandle<AIS_InteractiveObject> &object);
    static void AisContext_setObjectVisible(const OccHandle<AIS_InteractiveContext> &context,
                                            const OccHandle<AIS_InteractiveObject> &object,
                                            bool on);

    static AIS_InteractiveContext *
    AisObject_contextPtr(const OccHandle<AIS_InteractiveObject> &object);
    ...
    static void Gfx3dClipPlane_setCappingHatch(const OccHandle<Graphic3d_ClipPlane> &plane,
                                               Aspect_HatchStyle hatch);
    static void Gfx3dClipPlane_setNormal(const OccHandle<Graphic3d_ClipPlane> &plane,
                                         const gp_Dir &n);
    static void Gfx3dClipPlane_setPosition(const OccHandle<Graphic3d_ClipPlane> &plane, double pos);
