...
void V3dViewCameraAnimation::setBackend(std::unique_ptr<IAnimationBackend> anim)
{
    ...
    if (m_backend)
    {
        ...
        m_backend->setTimerCallback([this](long long t) { this->updateCurrentTime(t); });
    }
}
...
void V3dViewCameraAnimation::setDuration(long long t)
{
    m_duration = t;
    if (m_backend)
...
void V3dViewCameraAnimation::updateCurrentTime(long long currTime)
{
    if (!m_view)
        return;

    double progress = static_cast<double>(currTime) / static_cast<double>(m_duration);
    const double t = m_backend ? m_backend->valueForProgress(progress) : 0.;
