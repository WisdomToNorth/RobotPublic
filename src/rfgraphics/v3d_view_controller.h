...
public:
    ...
    void turn(V3d_TypeOfAxe axis, double radians);

    double instantZoomFactor() const
    {
        return m_instantZoomFactor;
    }
    void setInstantZoomFactor(double factor)
    {
        m_instantZoomFactor = factor;
    }

    // Signals
    rfapp::RfSignal<DynamicAction> signalDynamicActionStarted;
    rfapp::RfSignal<DynamicAction> signalDynamicActionEnded;
    rfapp::RfSignal<> signalViewScaled;
    rfapp::RfSignal<int, int> signalMouseMoved; // x,y: mouse position in view
    rfapp::RfSignal<Aspect_VKeyMouse> signalMouseButtonClicked;
    rfapp::RfSignal<bool> signalMultiSelectionToggled;
