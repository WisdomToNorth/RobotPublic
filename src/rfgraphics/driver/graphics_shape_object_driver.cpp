...
namespace
{
struct GraphicsShapeObjectDriverI18N
{
    MAYO_DECLARE_TEXT_ID_FUNCTIONS(Mayo::GraphicsShapeObjectDriver)
};
} // namespace
    ...
    this->setDisplayModes(
        {{DisplayMode_Wireframe, GraphicsShapeObjectDriverI18N::textId("Shape_Wireframe")},
         {DisplayMode_HiddenLineRemoval,
          GraphicsShapeObjectDriverI18N::textId("Shape_HiddenLineRemoval")},
         {DisplayMode_Shaded, GraphicsShapeObjectDriverI18N::textId("Shape_Shaded")},
         {DisplayMode_ShadedWithFaceBoundary,
          GraphicsShapeObjectDriverI18N::textId("Shape_ShadedWithFaceBoundary")}});
...
GraphicsObjectPtr GraphicsShapeObjectDriver::createObject(const TDF_Label &label) const
{
    if (XCaf::isShape(label))
    {
        ...
        // object->Attributes()->SetShadingModel(Graphic3d_TypeOfShadingModel_Pbr,
        // true/*overrideDefaults*/);
...
void GraphicsShapeObjectDriver::applyDisplayMode(GraphicsObjectPtr object,
                                                 Enumeration::Value mode) const
{
    ...
    else
    {
        ...
        const AIS_DisplayMode aisDispMode =
            mode == DisplayMode_Wireframe ? AIS_WireFrame : AIS_Shaded;
...
Enumeration::Value
GraphicsShapeObjectDriver::currentDisplayMode(const GraphicsObjectPtr &object) const
{
    this->throwIf_differentDriver(object);
    ...
    {
        return object->Attributes()->FaceBoundaryDraw() ? DisplayMode_ShadedWithFaceBoundary :
                                                          DisplayMode_Shaded;
    }
