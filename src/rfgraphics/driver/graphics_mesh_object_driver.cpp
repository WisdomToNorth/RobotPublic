...
namespace
{
struct GraphicsMeshObjectDriverI18N
{
    MAYO_DECLARE_TEXT_ID_FUNCTIONS(Mayo::GraphicsMeshObjectDriver)
};
} // namespace
...
GraphicsMeshObjectDriver::GraphicsMeshObjectDriver()
{
    this->setDisplayModes({
        ...
        {MeshVS_DMF_Shrink,
         GraphicsMeshObjectDriverI18N::textId("Mesh_Shrink")} // MeshVS_DA_ShrinkCoeff
    });
    this->setDefaultDisplayMode(MeshVS_DMF_Shading);
}

GraphicsMeshObjectDriver::Support
GraphicsMeshObjectDriver::supportStatus(const TDF_Label &label) const
{
...
void GraphicsMeshObjectDriver::applyDisplayMode(GraphicsObjectPtr object,
...
Enumeration::Value
GraphicsMeshObjectDriver::currentDisplayMode(const GraphicsObjectPtr &object) const
{
    this->throwIf_differentDriver(object);
    ...
    ObjectProperties(Span<const GraphicsObjectPtr> spanObject)
    {
        ...
        auto fnCheckState = [&](int count)
        {
            if (count == 0)
                ...
                return CppUtils::cmpEqual(count, spanObject.size()) ? CheckState::On :
    ...
    void onPropertyChanged(Property *prop) override
    {
            ...
            if (m_propertyShowEdges.value() != CheckState::Partially)
            {
                for (const Handle_MeshVS_Mesh &meshVisu : m_vecMeshVisu)
                {
                    meshVisu->GetDrawer()->SetBoolean(
                        MeshVS_DA_ShowEdges, m_propertyShowEdges.value() == CheckState::On);
        ...
        else if (prop == &m_propertyShowNodes)
        {
            if (m_propertyShowNodes.value() != CheckState::Partially)
            {
                ...
                {
                    meshVisu->GetDrawer()->SetBoolean(
                        MeshVS_DA_DisplayNodes, m_propertyShowNodes.value() == CheckState::On);
                    fnRedisplay(meshVisu);
                }
            }
        }
...
GraphicsMeshObjectDriver::Support
GraphicsMeshObjectDriver::meshSupportStatus(const TDF_Label &label)
{
