...
// Each graphics object "knows" the driver which created it: use function
// GraphicsObjectDriver::get()
...
public:
    ...
    Enumeration::Value defaultDisplayMode() const
    {
        return m_defaultDisplayMode;
    }
    const Enumeration &displayModes() const
    {
        return m_enumDisplayModes;
    }
    ...
    virtual std::unique_ptr<PropertyGroupSignals>
    properties(Span<const GraphicsObjectPtr> spanObject) const = 0;
...
protected:
    void setDisplayModes(Enumeration enumeration)
    {
        m_enumDisplayModes = std::move(enumeration);
    }
    void setDefaultDisplayMode(Enumeration::Value mode)
    {
