...
public:
    ...
    virtual void setDuration(long long t) = 0;
    virtual bool isRunning() const = 0;
    virtual void start() = 0;
    virtual void stop() = 0;
    virtual double valueForProgress(double p) const = 0; // 'p' in [0,1]
    virtual void setTimerCallback(std::function<void(long long)> fn) = 0;
};
...
public:
    ...
    const OccHandle<V3d_View> &view() const
    {
        return m_view;
    }
...
private:
    void updateCurrentTime(long long currTime);
    ...
    long long m_duration = 1; // 1 second
