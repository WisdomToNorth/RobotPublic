...
void V3dViewController::turn(V3d_TypeOfAxe axis, double radians)
{
    m_view->Turn(axis, radians, true /*start*/);
    this->redrawView();
}
...
void V3dViewController::startDynamicAction(DynamicAction dynAction)
{
    ...
    signalDynamicActionStarted.send(dynAction);
}
...
void V3dViewController::stopDynamicAction()
{
    if (m_dynamicAction != DynamicAction::None)
    {
        signalDynamicActionEnded.send(m_dynamicAction);
...
void V3dViewController::startInstantZoom(const Position &currPos)
{
    ...
    this->backupCamera();
    const int dX =(int) std::lround(m_instantZoomFactor * 100);
    m_view->StartZoomAtPoint(currPos.x, currPos.y);
