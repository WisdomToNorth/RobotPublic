...
double MeshUtils::triangulationVolume(const Handle_Poly_Triangulation &triangulation)
{
    ...
    for (const Poly_Triangle &tri : MeshUtils::triangles(triangulation))
    {
        int v1, v2, v3;
        tri.Get(v1, v2, v3);
        volume += MeshUtils::triangleSignedVolume(triangulation->Node(v1).Coord(),
...
double MeshUtils::triangulationArea(const Handle_Poly_Triangulation &triangulation)
{
    ...
    for (const Poly_Triangle &tri : MeshUtils::triangles(triangulation))
    {
        int v1, v2, v3;
        tri.Get(v1, v2, v3);
        area += MeshUtils::triangleArea(triangulation->Node(v1).Coord(),
                                        triangulation->Node(v2).Coord(),
                                        triangulation->Node(v3).Coord());
    }
...
void MeshUtils::setNode(const Handle_Poly_Triangulation &triangulation, int index,
                        const gp_Pnt &pnt)
{
    triangulation->SetNode(index, pnt);
}

void MeshUtils::setTriangle(const Handle_Poly_Triangulation &triangulation, int index,
                            const Poly_Triangle &triangle)
{
    triangulation->SetTriangle(index, triangle);
}

void MeshUtils::setNormal(const Handle_Poly_Triangulation &triangulation, int index,
...
MeshUtils::Orientation MeshUtils::orientation(const AdaptorPolyline2d &polyline)
{
    ...
    for (int i = 1; i < pntCount; ++i)
    {
        ...
        if (pnt.Y() < pntExtreme.Y() ||
            (rfbase::absEqual(pnt.Y(), pntExtreme.Y()) && (pnt.X() > pntExtreme.X())))
        {
            pntExtreme = pnt;
            indexPntExtreme = i;
        }
    ...
    const gp_Pnt2d beforeExtremePnt =
        polyline.pointAt((indexPntExtreme + (pntCount - 1)) % pntCount);
    ...
    const double triangleArea = a.X() * b.Y() - a.Y() * b.X() + a.Y() * c.X() - a.X() * c.Y() +
                                b.Y() * c.X() - c.X() * b.Y();
