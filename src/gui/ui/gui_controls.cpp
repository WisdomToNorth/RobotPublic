...
namespace rfgui
{
gui_controls::gui_controls(QWidget *parent) : QDialog(parent), ui(new Ui::gui_controls)
{
    ui->setupUi(this);
    ...
    QObject::connect(ui->doubleSpinBox_acceleration, SIGNAL(valueChanged(double)),
                     SLOT(update_accmax(double)));
    QObject::connect(ui->doubleSpinBox_velocity, SIGNAL(valueChanged(double)),
                     SLOT(update_velmax(double)));
    ...
    // Joints
    QObject::connect(ui->doubleSpinBox_joint_stepsize, SIGNAL(valueChanged(double)),
                     SLOT(update_joint_stepsize(double)));
    QObject::connect(ui->doubleSpinBox_joint_max_deg_sec, SIGNAL(valueChanged(double)),
                     SLOT(update_joint_maxdegsec(double)));

    QObject::connect(ui->pushButton_joint_deg_sec_plus, SIGNAL(pressed()),
                     SLOT(joint_deg_sec_plus()));
    QObject::connect(ui->pushButton_joint_deg_sec_min, SIGNAL(pressed()),
                     SLOT(joint_deg_sec_min()));
    ...
    QObject::connect(ui->toolButton_mdi_copy_cur_joint_pos, SIGNAL(pressed()),
                     SLOT(mdi_copy_cur_joint_pos()));
    ...
    QObject::connect(ui->toolButton_mdi_copy_cur_cart_pos, SIGNAL(pressed()),
                     SLOT(mdi_copy_cur_cart_pos()));
...
{
    mdi_cart(ui->lineEdit_mdi_cartx->text().toDouble(), ui->lineEdit_mdi_carty->text().toDouble(),
...
void gui_controls::tooldir_x_min()
{
    ...
    double x_out, y_out, z_out;
    int ok = kinematic().Fk_tooldir(ui->doubleSpinBox_tooldir_stepsize->value() * -1, 0, 0, x_out,
                                    y_out, z_out);
    if (ok)
    {
        mdi_cart(x_out, y_out, z_out, 1); // If point on axis if found, perform a move.
...
{
    double x_out, y_out, z_out;
    int ok = kinematic().Fk_tooldir(ui->doubleSpinBox_tooldir_stepsize->value(), 0, 0, x_out, y_out,
                                    z_out);
    if (ok)
    {
...
{
    double x_out, y_out, z_out;
    int ok = kinematic().Fk_tooldir(0, ui->doubleSpinBox_tooldir_stepsize->value(), 0, x_out, y_out,
                                    z_out);
    if (ok)
    {
        mdi_cart(x_out, y_out, z_out, 1);
...
{
    double x_out, y_out, z_out;
    int ok = kinematic().Fk_tooldir(0, ui->doubleSpinBox_tooldir_stepsize->value() * -1, 0, x_out,
                                    y_out, z_out);
    if (ok)
    {
        mdi_cart(x_out, y_out, z_out, 1);
...
{
    double x_out, y_out, z_out;
    int ok = kinematic().Fk_tooldir(0, 0, ui->doubleSpinBox_tooldir_stepsize->value(), x_out, y_out,
                                    z_out);
    if (ok)
    {
        mdi_cart(x_out, y_out, z_out, 1);
...
{
    double x_out, y_out, z_out;
    int ok = kinematic().Fk_tooldir(0, 0, ui->doubleSpinBox_tooldir_stepsize->value() * -1, x_out,
                                    y_out, z_out);
    if (ok)
    {
        mdi_cart(x_out, y_out, z_out, 1);
...
void gui_controls::mdi_cart(double x, double y, double z, bool tooldir)
{
    ...
    if (ui->cb_mode_fb->isChecked())
    {
        ...
        cart.p.z(z);
        if (!tooldir)
        { // When in tooldir, don't change euler angles.
...
void gui_controls::mdi_joint_go()
{
    ...
    if (j0_rad > KDLJointMin(0) && j0_rad < KDLJointMax(0) && j1_rad > KDLJointMin(1)
        && j1_rad < KDLJointMax(1) && j2_rad > KDLJointMin(2) && j2_rad < KDLJointMax(2)
        && j3_rad > KDLJointMin(3) && j3_rad < KDLJointMax(3) && j4_rad > KDLJointMin(4)
        && j4_rad < KDLJointMax(4) && j5_rad > KDLJointMin(5) && j5_rad < KDLJointMax(5))
    {
        // If move is within the joint limits, go on.

        if (ui->cb_mode_fb->isChecked())
        { // Live mode
            int ok = jog().jog_mdi_joint(
                j0, j1, j2, j3, j4, j5,
                ui->doubleSpinBox_mdi_joint_max_deg_sec->value()); // Create stream
            if (ok)
            {
...
void gui_controls::on_cb_mode_fb_toggled(bool checked)
{
    if (checked)
    {
        mode_feedback = 1;
...
void gui_controls::on_cb_mode_ik_from_init_toggled(bool checked)
{
    if (checked)
    {
        mode_ikfrominit = 1;
