...
//
// Data type 'T' must be default-constructible(see
// https://www.cplusplus.com/reference/type_traits/is_default_constructible/)
//
template <typename T>
class RfIdTree
{
public:
    ...
    // Data associated to node of identifier 'id' or default-constructed object of type 'T' in case
    // of error
    ...
    std::span<const TreeNodeId> roots() const;
...
private:
    ...
    friend void traverseTree_unorder(const RfIdTree<U> &tree, const FN &callback);

    template <typename U, typename FN>
    friend void traverseTree_preOrder(TreeNodeId node, const RfIdTree<U> &tree, const FN &callback);

    template <typename U, typename FN>
    friend void traverseTree_postOrder(TreeNodeId node, const RfIdTree<U> &tree,
                                       const FN &callback);

    template <typename U, typename FN>
    friend void visitDirectChildren(TreeNodeId id, const RfIdTree<U> &tree, const FN &callback);
...
void traverseTree_unorder(const RfIdTree<T> &tree, const FN &callback);

template <typename T, typename FN>
void traverseTree_preOrder(const RfIdTree<T> &tree, const FN &callback);

template <typename T, typename FN>
void traverseTree_preOrder(TreeNodeId id, const RfIdTree<T> &tree, const FN &callback);

template <typename T, typename FN>
void traverseTree_postOrder(const RfIdTree<T> &tree, const FN &callback);

template <typename T, typename FN>
void traverseTree_postOrder(TreeNodeId id, const RfIdTree<T> &tree, const FN &callback);

template <typename T, typename FN>
void traverseTree(const RfIdTree<T> &tree, const FN &callback,
                  TreeTraversal mode = TreeTraversal::PreOrder);

template <typename T, typename FN>
void traverseTree(TreeNodeId id, const RfIdTree<T> &tree, const FN &callback,
                  TreeTraversal mode = TreeTraversal::PreOrder);

template <typename U, typename FN>
void visitDirectChildren(TreeNodeId id, const RfIdTree<U> &tree, const FN &callback);
...
template <typename T>
TreeNodeId RfIdTree<T>::nodeSiblingPrevious(TreeNodeId id) const
{
...
template <typename T>
TreeNodeId RfIdTree<T>::nodeSiblingNext(TreeNodeId id) const
{
    const TreeNode *node = this->ptrNode(id);
...
template <typename T>
TreeNodeId RfIdTree<T>::nodeChildFirst(TreeNodeId id) const
{
    const TreeNode *node = this->ptrNode(id);
...
template <typename T>
TreeNodeId RfIdTree<T>::nodeChildLast(TreeNodeId id) const
{
    const TreeNode *node = this->ptrNode(id);
...
template <typename T>
TreeNodeId RfIdTree<T>::nodeParent(TreeNodeId id) const
{
    const TreeNode *node = this->ptrNode(id);
...
template <typename T>
TreeNodeId RfIdTree<T>::nodeRoot(TreeNodeId id) const
{
...
template <typename T>
const T &RfIdTree<T>::nodeData(TreeNodeId id) const
{
...
template <typename T>
bool RfIdTree<T>::nodeIsRoot(TreeNodeId id) const
{
    const TreeNode *node = this->ptrNode(id);
...
template <typename T>
bool RfIdTree<T>::nodeIsLeaf(TreeNodeId id) const
{
    return this->nodeChildFirst(id) == 0;
}

template <typename T>
void RfIdTree<T>::clear()
{
    m_vecNode.clear();
    m_vecRoot.clear();
}
...
template <typename T>
TreeNodeId RfIdTree<T>::appendChild(TreeNodeId parentId, const T &data)
{
...
template <typename T>
TreeNodeId RfIdTree<T>::appendChild(TreeNodeId parentId, T &&data)
{
    TreeNode *node = this->appendChild(parentId);
...
template <typename T>
typename RfIdTree<T>::TreeNode *RfIdTree<T>::appendChild(TreeNodeId parentId)
{
    m_vecNode.push_back({});
...
template <typename T>
bool RfIdTree<T>::isNodeDeleted(TreeNodeId id) const
{
...
template <typename T>
void RfIdTree<T>::removeRoot(TreeNodeId id)
{
...
template <typename T>
std::span<const TreeNodeId> RfIdTree<T>::roots() const
{
    // return m_vecRoot;
    return std::span<const TreeNodeId>(m_vecRoot.data(), m_vecRoot.size());
}

template <typename T>
TreeNodeId RfIdTree<T>::lastNodeId() const
{
    return static_cast<TreeNodeId>(m_vecNode.size());
}

template <typename T>
typename RfIdTree<T>::TreeNode *RfIdTree<T>::ptrNode(TreeNodeId id)
{
    return id != 0 && id <= m_vecNode.size() ? &m_vecNode.at(id - 1) : nullptr;
}

template <typename T>
const typename RfIdTree<T>::TreeNode *RfIdTree<T>::ptrNode(TreeNodeId id) const
{
    return id != 0 && id <= m_vecNode.size() ? &m_vecNode.at(id - 1) : nullptr;
}

template <typename T, typename FN>
void traverseTree(const RfIdTree<T> &tree, const FN &callback, TreeTraversal mode)
{
    switch (mode)
    {
    case TreeTraversal::Unorder: return traverseTree_unorder(tree, callback);
    case TreeTraversal::PreOrder: return traverseTree_preOrder(tree, callback);
    case TreeTraversal::PostOrder: return traverseTree_postOrder(tree, callback);
    }
}

template <typename T, typename FN>
void traverseTree(TreeNodeId id, const RfIdTree<T> &tree, const FN &callback, TreeTraversal mode)
{
    switch (mode)
    {
    case TreeTraversal::Unorder:
    case TreeTraversal::PreOrder: return traverseTree_preOrder(id, tree, callback);
    case TreeTraversal::PostOrder: return traverseTree_postOrder(id, tree, callback);
    }
}
...
void traverseTree_unorder(const RfIdTree<T> &tree, const FN &callback)
{
    for (const typename RfIdTree<T>::TreeNode &node : tree.m_vecNode)
    {
...
void visitDirectChildren(TreeNodeId id, const RfIdTree<U> &tree, const FN &callback)
{
    ...
    for (auto idChild = tree.nodeChildFirst(id); idChild != 0;
         idChild = tree.nodeSiblingNext(idChild))
    {
